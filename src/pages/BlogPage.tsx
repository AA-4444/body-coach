...
const categoryMap: Record<FilterCategory, OriginalCategory> = {
  ПОСЛЕДНЕЕ: "LATEST",
...
const BlogPage = () => {
  const [activeCategory, setActiveCategory] =
    useState<FilterCategory>("ПОСЛЕДНЕЕ");

  const filteredPosts = useMemo(() => {
    if (activeCategory === "ПОСЛЕДНЕЕ") return blogPosts;

    return blogPosts.filter(
      (post) => post.category === categoryMap[activeCategory]
    );
  }, [activeCategory]);

  const featuredPost =
    activeCategory === "ПОСЛЕДНЕЕ"
      ...
      <section className="relative overflow-hidden bg-[#83dce4]">
        ...
        <div className="mx-auto w-full max-w-[1440px] px-5 lg:px-10 xl:px-14 py-16 lg:py-20">
          <div className="min-h-[170px] lg:min-h-[210px] flex items-center justify-center">
            <Reveal
              as="h1"
              className="heading-hero text-[#1557d6] text-5xl sm:text-6xl lg:text-[5rem] text-center leading-[1]"
            >
              БЛОГ ВИКТОРИИ
            </Reveal>
          </div>
      ...
      {featuredPost && (
        <section className="pt-12 lg:pt-16">
          <div className="mx-auto w-full max-w-[1080px] px-5 lg:px-8">
            <Reveal>
              <article className="[content-visibility:auto] [contain-intrinsic-size:900px]">
                <Link to={`/blog/${featuredPost.slug}`} className="block group">
                  <div className="overflow-hidden rounded-[38px] mb-8 lg:mb-10 bg-slate-200">
                    <img
                      src={featuredPost.image}
                      alt={featuredPost.title}
                      className="w-full h-[280px] sm:h-[380px] lg:h-[430px] object-cover"
                      loading="eager"
                      decoding="async"
                      draggable={false}
                    />
                  ...
                  <p className="font-heading font-bold tracking-[0.14em] uppercase text-[#334f81] text-[0.95rem] mb-3">
                    {categoryLabelMap[featuredPost.category] ??
                      featuredPost.category}
                  </p>

                  <h2
                    lang="ru"
                    className="heading-hero text-[#314f85] text-3xl sm:text-4xl lg:text-[3.1rem] leading-[1.12] mb-4 max-w-[900px] break-words hyphens-auto"
                  >
                    {featuredPost.title}
                  </h2>

                  <p className="text-[#334f81] text-lg sm:text-xl lg:text-[1.45rem] leading-snug max-w-[760px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-8 gap-y-14 lg:gap-x-10 lg:gap-y-16">
            {gridPosts.map((post, index) => (
              <Reveal
                ...
                className="min-w-0 [content-visibility:auto] [contain-intrinsic-size:620px]"
              >
                <article className="min-w-0">
                  ...
                  >
                    <div className="overflow-hidden rounded-[24px] mb-5 bg-slate-200">
                      <img
                        src={post.image}
                        alt={post.title}
                    ...
                    <p className="font-heading font-bold tracking-[0.14em] uppercase text-[#334f81] text-[0.85rem] mb-2">
                      {categoryLabelMap[post.category] ?? post.category}
                    </p>

                    <h3
                      lang="ru"
                      className="heading-hero text-[#314f85] text-[1.85rem] sm:text-[2rem] lg:text-[2.15rem] leading-[1.14] mb-4 max-w-full break-words hyphens-auto"
                    >
                      {post.title}
                    </h3>

                    <p className="text-[#334f81] text-base lg:text-[1.02rem] leading-relaxed max-w-[95%]">
                      {post.excerpt}
                    </p>
                  </Link>
                </article>
